...
import pandas as pd
import random
import start_loja_db_SVM3 as stl
...
list_aq = pd.read_csv(r"{}".format(diret)+"CSVLojaSVM3/list_aq.csv")
list_aq = list_aq.to_numpy()
...
def repoe_estoque(minimo):
    """ Insere produtos no estoque caso seja menor que o mínimo"""

    query="SELECT idProduto FROM produto WHERE prodEstoque<={}".format(minimo)
    df = pd.read_sql(query, mydb)

    for prod_id in np.array(df.values).flatten():
        # query_custo="SELECT prodCusto FROM produto WHERE idProduto = {}".format(prod_id)
        # val = pd.read_sql(query_custo, mydb)
        # val = val.to_numpy().flatten()
        
        val = (list_aq[prod_id-1])[2]
...
df_save.to_csv(r'C:\Users\sucod\OneDrive\Lenovo\Documentos\Sql\CSVLojaSVM3\venda_{}.csv'.format("svm3"))
query_save="SELECT * FROM produto"
df_save = pd.read_sql(query_save, mydb)
df_save.to_csv(r'C:\Users\sucod\OneDrive\Lenovo\Documentos\Sql\CSVLojaSVM3\produto_{}.csv'.format("svm3"))
query_save="SELECT * FROM departamento"
df_save = pd.read_sql(query_save, mydb)
df_save.to_csv(r'C:\Users\sucod\OneDrive\Lenovo\Documentos\Sql\CSVLojaSVM3\departamento_{}.csv'.format("svm3"))
query_save="SELECT * FROM aquisicao"
df_save = pd.read_sql(query_save, mydb)
df_save.to_csv(r'C:\Users\sucod\OneDrive\Lenovo\Documentos\Sql\CSVLojaSVM3\aquisicao_{}.csv'.format("svm3"))

print("Arquivos Salvos -- {}".format("svm3"))